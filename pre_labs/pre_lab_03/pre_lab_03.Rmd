...
It looks like 20001 has the most grant money, which is Washington DC. I guess it makes sense since it's a big tourist destination and people visiting DC can stay in Maryland, but it's a little weird that it's not something actually in the state like Ocean City, Annapolis or Baltimore.

```{r}
cleaned_md_grants_loans |> 
  group_by(amount, zip5) |>
arrange(desc(amount))
```

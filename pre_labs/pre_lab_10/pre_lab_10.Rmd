...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** This was just the libraries? Nothing happened aside from the libraries getting activated...
...
**Task** Run the following code. Describe the output in the space below. **Answer** This produced two new dataframes. One was a four column table sorting forclosure notices by county, the other was a table showing the populations of Maryland counties.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It's the same Maryland county population table from the previous code block, only this time it added the Maryland county georgraphy as a new column. It's also a MULTIPOLYGON data type, meaning it can be used with ggplot for mapping.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This displayed the two dataframes, where you can see the issue of the column names not matching. This needs to be fixed ahead of the join and map steps.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This just cleaned up the population table so that it's usuable and joinable with our county notices data.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Tables joined by the county column, now we can see the foreclosure numbers alongside the population for each Maryland county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This mutated in a new column on the previous table, showing the rate of foreclosures for each county in case there's a certain one that should be highlighted in the visualization.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This shows a map of Maryland counties colored based on their forclosure notice rates. The lighter the shade of blue, the higher the rate of forecolsure notices per 1000 residents. With the key, we can see that Charles County has the highest rate.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This changed up the color scheme from the previous map to add some more flare to the color code. It also makes it easier to see the differences between high and low rates, using two very different colors instead of doing a gradient fade of blues.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** This messed with the colors just a bit more, making it distinctly differt not just between high and low rates, but it also eliminated for of the "gray area" for counties with close rates that had similar shades on the gradient. As far as regional patterns, the higher rates tend to be in the southern chunk of the state, while the majority of the lower rates are in the north or on the eastern shore.

...
**First thing that stands out is 2022 had more words with sentiment than 2023, (824 to 519). Additionally, both datasets have negative sentiment words outnumber positive sentiment ones. As far as the words themselves, words like "limited" and "plea" are listed with a negative sentiment and those both feel like words that need extra context to determine their connotation. Same thing on the other side, words like "substantive" and "intimate" were listed as positive but likely need extra context in this situation.**
...
bing_word_counts_2023 |> group_by(sentiment) |> summarise(sum(n))

bing_word_counts_2022 |> group_by(sentiment) |> summarise(sum(n))
```
